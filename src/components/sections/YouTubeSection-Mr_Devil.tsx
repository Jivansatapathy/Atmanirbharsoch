...
// --- OPTION 2: CINEMA STREAM ---
const VariantStream: React.FC = () => {
    const [hoveredId, setHoveredId] = useState<string | null>(null);
    return (
        <div style={{ overflowX: 'auto', paddingBottom: '50px', marginBottom: '100px' }} className="hide-scrollbar">
            <div className="container">
                <div style={{ color: '#666', marginBottom: '20px', fontFamily: 'monospace' }}>// OPTION 02: CINEMA STREAM</div>
            </div>
            <div style={{ display: 'flex', paddingLeft: 'max(20px, (100vw - 1200px) / 2)', gap: '40px' }}>
                {videos.map((video) => (
                    <div
                        key={video.id}
                        onMouseEnter={() => setHoveredId(video.id)}
                        ...
                        style={{
                            minWidth: '600px',
                            cursor: 'pointer',
                            opacity: hoveredId && hoveredId !== video.id ? 0.3 : 1,
                    ...
                    >
                        <div style={{
                            ...
                            <div style={{ width: '100%', height: '100%', backgroundColor: video.thumbnail, transform: hoveredId === video.id ? 'scale(1.05)' : 'scale(1)', transition: 'transform 0.5s ease', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <Play size={48} color={hoveredId === video.id ? 'var(--color-red-ops)' : '#fff'} style={{ opacity: hoveredId === video.id ? 1 : 0.5, transition: 'all 0.3s' }} />
                            </div>
                            {/* Corner Brackets */}
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '10px' }}>
                                <span style={{ fontFamily: 'monospace', color: 'var(--color-red-ops)' }}>{video.id}</span>
                                <span style={{ height: '1px', flex: 1, backgroundColor: '#333' }}></span>
                                <span style={{ fontFamily: 'monospace', color: '#666' }}>{video.duration}</span>
                            </div>
                            <h3 style={{ fontSize: '2rem', margin: 0 }}>{video.title}</h3>
                        </div>
                    </div>
                ))}
            </div>
